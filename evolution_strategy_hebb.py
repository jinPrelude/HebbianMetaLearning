...
class EvolutionStrategyHebb(object):
    ...
    def run(self, iterations, print_step=10, path='heb_coeffs', log=True):                                                    
        
        id_ = str(int(time.time()))
        if log:
            wandb.init(project="hebbian")
            wandb.run.name = self.environment + "/" + id_
